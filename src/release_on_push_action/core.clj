...
(defn input-strategy-set? []
  (if (System/getenv "INPUT_STRATEGY")
    (do
      (println "WARNING: the action property `strategy` has been renamed `bump_version_scheme`. Support for `strategy` will be removed in the future. See the afeick/github-release-on-push-action README for the current configuration")
      true)
    false))
...
(defn norelease-reason [context related-data]
  (cond
    ...
    (contains? (get-labels (get-in related-data [:related-prs])) "no version")
    "Skipping release. Reason: related PR has label 'no version'"))
